...
#define RELOC(x) ((x) - KERNBASE)
	boot_map_region(kern_pgdir, KSTACKTOP - KSTKSIZE, KSTKSIZE,
            RELOC((uint32_t)bootstack), PTE_W | PTE_P);
	...
	// notice -KERNBASE here  !!
	// it is just the size of KERNBASE ~ 4G
...
mem_init_mp(void)
{
	...
	// for cpu 0 will remap the stack
	int i;
	for(i = 0; i < NCPU; ++i) {
	...
	// do not map [kstacktop_i - (KSTKSIZE + KSTKGAP), kstacktop_i - KSTKSIZE)
	// will fault and then do it in pgfault
}
...
page_init(void)
{
	...
	int num = PGNUM(MPENTRY_PADDR);
	for (i = 1 ; i < npages_basemem; ++i) {
		if(i == num) continue;  // lab4
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];
    }
	// if npages_basemem is equal to IOPHYSMEM
	// the left is free
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (!(*pde & PTE_P)) {  // not exists
		if(!create){        // do not  create
			pte =  NULL;
		}
		else {
			struct Page* page_table = page_alloc(ALLOC_ZERO);
			if(page_table == NULL) return NULL;
			++page_table->pp_ref;
			(*pde) = page2pa(page_table) | PTE_P | PTE_W | PTE_U;  
			// need more flags?  yes need PTE_U 
			// else error occur visiting envs from user space
			pte = &((pte_t*)page2kva(page_table))[PTX(va)];
		}
	}
	else {
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	for( ; size > 0; size -= PGSIZE) {
		...
		if(!pte) panic("%s : pgdir_walk failed\n", __func__);
		//if(*pte & PTE_P) panic("%s: pte remap\n", __func__);
		*pte = pa | perm | PTE_P;
		va += PGSIZE;
		pa += PGSIZE;
	}
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm)
{
	...
	// if a valid page table entry, remove that page
	// and set the page noe present
	pte_t* pte;
	...
	if(!(*pde & PTE_P)) {
		...
		++page_table->pp_ref;
		(*pde) = page2pa(page_table) | perm | PTE_P;
		//pte = (pte_t*)page2kva(page_table);
		pte = &((pte_t*)page2kva(page_table))[PTX(va)];
	}
	else {
		*pde |= perm;  // ok ??
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	if (!(pte[PTX(va)] & PTE_P)) return NULL;
	//if (pte_store != NULL) *pte_store = ptp;
	if (pte_store != NULL) *pte_store = &pte[PTX(va)];
...
page_remove(pde_t *pgdir, void *va)
{
	...
	assert(pte);
	//ptp[PTX(va)] &= (~PTE_P); // set the page not present
	*pte &= (~PTE_P); // set the page not present
	page_decref(page);
    tlb_invalidate(pgdir, va);
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for( ; start < end; start += PGSIZE) {
		if(start >= ULIM) { goto err; }
		pte = pgdir_walk(env->env_pgdir, (void*)start, 0);
		if(pte == NULL)   { cprintf("pte is NULL\n");goto err; }
		if( ((*pte) & perm) != perm) { cprintf("perm wrong\n"); goto err; }
	}
	return 0;
