...
#if 0
// this is for lab3, when lab4 comment it
#if defined(TEST)
...
//	pci_init();
	...
	boot_aps();  // will finally call mp_main();

	// Should always have idle processes at first.
	int i;
	for (i = 0; i < NCPU; i++)   // all idle for 0 ~ NCPU-1
...
mp_main(void)
{
	...
	//
	// Your code here:
	lock_kernel(); // need to lock kernrl first
	sched_yield(); // ok ??
